...
import { updateIdea } from "@/lib/actions/update-inovasi";
...
export function EditForm({ idea }: { idea: any }) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      nama: idea.nama || "",
      bidang: idea.bidang || "",
      subBidang: idea.subBidang || "",
      jumlahAnggota: idea.jumlahAnggota ? String(idea.jumlahAnggota) : "",
      kategoriBidangInovasi: idea.kategoriBidangInovasi || "",
      judul: idea.judul || "",
      kategoriInovasi: idea.kategoriInovasi || "",
      latarBelakang: idea.latarBelakang || "",
      implementasi: idea.implementasi || "",
      manfaat: idea.manfaat || "",
      peluangDiseminasi: idea.peluangDiseminasi || "",
    },
  });
    ...
    try {
      ...
      if (result.success) {
        toast.success("Perubahan Berhasil Disimpan!", {
          description: "Data inovasi Anda telah diperbarui.",
        });
        router.push(`/board/${idea.id}`);
        router.refresh();
      } else {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 md:space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-700 pb-20 px-3">
      <div className="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6 text-center md:text-left">
        ...
        <div>
          <h1 className="text-xl md:text-4xl font-black tracking-tighter text-slate-900 uppercase">Edit Innovation</h1>
          <p className="text-slate-500 font-medium mt-1 text-xs md:text-base">
            Perbarui data inovasi Anda jika ada perubahan.
          </p>
        </div>
      </div>
          ...
          <div className="glass-card p-5 md:p-10 rounded-[1.5rem] md:rounded-[2.5rem] space-y-6 md:space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
               ...
               <div 
                  ...
                  className="p-6 rounded-[2rem] border-2 border-dashed border-slate-100 flex flex-col items-center justify-center text-center space-y-2 bg-slate-50/30 group hover:border-teal-200 hover:bg-teal-50/10 transition-all cursor-pointer"
               >
                  <div className="h-10 w-10 rounded-full bg-white shadow-sm flex items-center justify-center mb-2">
               ...
               <div 
                  onClick={() => ecpRef.current?.click()}
                  className="p-6 rounded-[2rem] border-2 border-dashed border-slate-100 flex flex-col items-center justify-center text-center space-y-2 bg-slate-50/30 group hover:border-rose-200 hover:bg-rose-50/10 transition-all cursor-pointer"
               >
                  <div className="h-10 w-10 rounded-full bg-white shadow-sm flex items-center justify-center mb-2">
